  ...
  describe('register', () => {
    it('should throw ConflictException when username is existed ', async () => {
      ...
      try {
        await lastValueFrom(
          controller.register(
            { username: 'amr' } as RegisterDto,
            responseMock,
          ),
        );
      } catch (e) {
        expect(e).toBeDefined();
        expect(existsByUsernameSpy).toBeCalledWith('amr');
    ...
    it('should throw ConflictException when email is existed ', async () => {
      ...
      try {
        await lastValueFrom(
          controller.register(
            { username: 'amr', email: 'amr@example.com' } as RegisterDto,
            responseMock,
          ),
        );
      } catch (e) {
        expect(e).toBeDefined();
        expect(existsByUsernameSpy).toBeCalledWith('amr');
        expect(existsByEmailSpy).toBeCalledWith('amr@example.com');
        expect(saveSpy).toBeCalledTimes(0);
      }
    });
